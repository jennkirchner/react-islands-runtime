...
const warnMissingConfig = () => {
	...
	console.warn('[contentstack-commercetools] Missing CONTENTSTACK_* env; using built-in local content.');
};
...
const warnCmsError = (error) => {
	...
	console.warn(
		'[contentstack-commercetools] Falling back to built-in local content after CMS error:',
		error?.message || error,
	);
};

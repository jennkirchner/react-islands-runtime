...
const warnMissingConfig = () => {
	...
	console.warn('[contentstack] Missing CONTENTSTACK_* env; using built-in local content.');
};
...
const warnCmsError = (error) => {
	...
	console.warn('[contentstack] Falling back to built-in local content after CMS error:', error?.message || error);
};
	...
	blocks: [
		{
			type: 'hero',
			title: 'Weekly Deals',
			subtitle: 'Running with built-in local content until Contentstack is configured.',
		},
		{
			type: 'product_search',
